...
{
    private IntegrationTestUserEntity? _currentUserEntity { get; set; }
    ...
    public IntegrationTestUserEntity? CurrentUserEntity => this._currentUserEntity;
    ...
    public async Task InitializeUser(IntegrationTestBase testApplicationFactory)
    {
        ...
        this._currentUserEntity = await this.CreateUser(testApplicationFactory, this._role.Value);
    }
        ...
        UserRoleEnum role, bool withSignIn = true)
    {
        using (var scope = testApplicationFactory.Services.CreateScope())
        {
            ...
            user.Salt = jwtTokenFactory.GenerateSalt();
            user.PasswordHash = jwtTokenFactory.HashPassword(IntegrationTestConstants.UserName, user.Salt);
            ...
            if (withSignIn)
            {
                this._currentUserEntity = result;
                this._role = role;
                
                var token = jwtTokenFactory.GenerateJwtToken(
                    this._currentUserEntity.User.Id,
                    this._currentUserEntity.User.Login,
                    this._currentUserEntity.User.Email,
                    string.Join(",", this._role.Value.ToString()),
                    true);
    ...
    private List<Claim> GetTestClaims()
    {
        ...
        List<Claim> claims = new List<Claim>
        {
            ...
            new Claim(AuthClaims.Role, this.CurrentUserEntity.User.Roles.FirstOrDefault().Role.UserRole.ToString()),
