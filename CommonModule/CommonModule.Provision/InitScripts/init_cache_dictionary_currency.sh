...
echo "Caching currencies in Redis..."
...
cache_table_data() {
  ...
  psql -h $db_host -p $db_port -d $db_name -U $db_user -t -A -F $'\t' -c "$sql" | while IFS=$'\t' read -r id title code symbol titleEn isActive
  do
    ...
    if redis-cli -h $redis_host -p $redis_port SET "$key" "{\"id\":\"$id\",\"title\":\"$title\",\"code\":\"$code\",\"symbol\":\"$symbol\",\"titleEn\":\"$titleEn\",\"isActive\":$isActiveValue}"; then
...
# Calculate total count for currencies
totalCountCurrencies=$(calculate_total_count "SELECT COUNT(*) FROM \"Dictionaries\".\"Currencies\";")

# Cache currencies
cache_table_data "Currencies" "dictionary_cache:currency:{ID}" "SELECT \"Id\", \"Title\", \"Code\", \"Symbol\", \"TitleEn\", \"IsActive\" FROM \"Dictionaries\".\"Currencies\";" "$totalCountCurrencies"
...
"$SCRIPT_DIR/add_logs.sh" "CACHE CURRENCIES" "$totalCountCurrencies" "0" "$errorAdded"