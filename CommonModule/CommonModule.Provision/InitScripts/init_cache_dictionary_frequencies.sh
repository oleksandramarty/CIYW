...
echo "Caching frequencies in Redis..."
...
cache_table_data() {
  ...
  psql -h $db_host -p $db_port -d $db_name -U $db_user -t -A -F $'\t' -c "$sql" | while IFS=$'\t' read -r id title description isActive type
  do
    ...
    if redis-cli -h $redis_host -p $redis_port SET "$key" "{\"id\":\"$id\",\"title\":\"$title\",\"description\":\"$description\",\"isActive\":$isActiveValue,\"type\":\"$type\"}"; then
...
# Calculate total count for frequencies
...
"$SCRIPT_DIR/add_logs.sh" "CACHE FREQUENCIES" "$totalCountFrequencies" "0" "$errorAdded"
echo "Frequencies cached in Redis."
