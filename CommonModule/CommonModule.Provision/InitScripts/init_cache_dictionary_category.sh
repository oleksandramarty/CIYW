...
echo "Caching categories in Redis..."
...
cache_table_data() {
  ...
  psql -h $db_host -p $db_port -d $db_name -U $db_user -t -A -F $'\t' -c "$sql" | while IFS=$'\t' read -r id title icon color isActive parentId isPositive
  do
    ...
    if redis-cli -h $redis_host -p $redis_port SET "$key" "{\"id\":\"$id\",\"title\":\"$title\",\"icon\":\"$icon\",\"color\":\"$color\",\"isActive\":$isActiveValue,\"parentId\":$parentIdValue,\"isPositive\":$isPositiveValue}"; then
...
totalCountCategories=$(calculate_total_count "SELECT COUNT(*) FROM \"Dictionaries\".\"Categories\";")

# Cache categories
cache_table_data "Categories" "dictionary_cache:category:{ID}" "SELECT \"Id\", \"Title\", \"Icon\", \"Color\", \"IsActive\", \"ParentId\", \"IsPositive\" FROM \"Dictionaries\".\"Categories\";" "$totalCountCategories"
...
"$SCRIPT_DIR/add_logs.sh" "CACHE CATEGORIES" "$totalCountCategories" "0" "$errorAdded"