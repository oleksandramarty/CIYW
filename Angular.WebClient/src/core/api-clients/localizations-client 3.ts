...
export class LocalizationClient {
    ...
    localization_GetLocales(request: GetLocalesRequest): Observable<VersionedListResponseOfLocaleResponse> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<VersionedListResponseOfLocaleResponse>;
                }
            } else
                return _observableThrow(response_) as any as Observable<VersionedListResponseOfLocaleResponse>;
        }));
    }

    protected processLocalization_GetLocales(response: HttpResponseBase): Observable<VersionedListResponseOfLocaleResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 400) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = VersionedListResponseOfLocaleResponse.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
